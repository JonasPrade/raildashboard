...
def upgrade() -> None:
    # vib_report: one row per annual PDF report
    op.create_table(
        "vib_report",
    ...
    # vib_entry: one row per Vorhaben (section B.4.x.x) in a report
    op.create_table(
        "vib_entry",
        sa.Column("id", sa.Integer(), primary_key=True, index=True),
        sa.Column(
    ...
    # vib_pfa_entry: rows from the PFA table inside a vib_entry
    op.create_table(
        "vib_pfa_entry",
        sa.Column("id", sa.Integer(), primary_key=True, index=True),
        sa.Column(
