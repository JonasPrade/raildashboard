...
>[!important]
>
>the ERA data has no geo data -> the project data is not as beautiful as supposted
...
- [ ] Connection with openstreetmap data
	- Another idea would be to use the open data from OpenStreetMap as base for the railway infrastructure. The good thing is that it has more than Europe.
	- the problem is that it has too many details and is not as good for handling in for example routing queries
- [ ] Connection with DB OpenData
	- https://www.govdata.de/suche/daten/schienennetz-deutsche-bahnddea3
	- -> das scheint nicht wirklich aktualisiert zu werden 
